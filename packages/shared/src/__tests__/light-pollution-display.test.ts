...
describe("public sky darkness display", () => {
  ...
  it("contains no location, coordinate, or category-specific production rule", () => {
    ...
    expect(source).not.toMatch(/黄山|光明顶|三清山|老君山|武功山|Tianwentong|天文通/);
    expect(source).not.toMatch(/latitude|longitude|locationName|scenic|景区|城市|农村|乡村|山地/);
    expect(source).not.toMatch(/category|country|region|province/i);
  });
});
