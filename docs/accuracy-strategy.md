...
- Use `bash scripts/diagnose-sky-darkness.sh --coordinate lat,lon --json` for coordinate-level WA/VIIRS diagnostics and `bash scripts/evaluate-sky-darkness-benchmarks.sh <reference.csv> --format json --include-coordinates` for QA-only public-fusion benchmark reports. The benchmark wrapper also accepts `--json` as a compatibility alias for `--format json`.
