...
Milky Way light-pollution scoring V1:
...
- 原始 VIIRS 波特尔估算只作为诊断保留；公开页面使用全局保守展示层。缺少足够校准证据、采样不足、低置信度或周边光穹与本地辐亮度不一致时，公开范围会放宽，尤其不能把未经校准的 1-2 级直接展示成极佳暗空。
- 估算波特尔只显示范围，是因为当前输入来自卫星夜光和环境风险指数，不是现场校准测量。没有独立校准前，不显示 SQM、国标等级、官方等级或单一实测波特尔级。
