...
- Use `bash scripts/diagnose-sky-darkness.sh --coordinate lat,lon --json` for coordinate-level WA/VIIRS diagnostics and `bash scripts/report-sky-darkness-qa.sh <reference.csv> --include-coordinates` for QA-only public-fusion benchmark reports. The report wrapper writes Markdown and JSON by default; the older benchmark wrapper remains available for compatibility.
