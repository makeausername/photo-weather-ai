...
function pickAstroHourlyFields(
...
> {
  return {
    ...
    calendarBasis: result.calendarBasis,
    professionalHourlyData: result.professionalHourlyData,
    professionalHourlyDataTimeBasis: result.professionalHourlyDataTimeBasis,
  };
}
...
describe("forecast result target-aware view model", () => {
  ...
  it("renders terrain horizon status on the astro page without expanding professional diagnostics", () => {
    ...
    expect(viewModel.publicDisplay.factorChips.map((chip) => chip.key)).toContain(
      "terrain-horizon",
    );
  ...
  it("uses selected Milky Way DEM geometry as the canonical terrain display", () => {
    ...
    expect(terrainFactor).toBeUndefined();
    expect(
  ...
  it("shows multiple daily glow entries for a 7d glow result", () => {
    ...
    const html = renderToStaticMarkup(
      React.createElement(GlowResultPage, {
        query: { ...queryForTarget("glow"), horizon: "7d" },
  ...
  it("builds the canonical observing-night astro model from deterministic facts", () => {
    ...
    expect(astro?.judgmentFactors.map((factor) => factor.label)).not.toContain("地形遮挡");
    expect(astro?.judgmentFactors.map((factor) => factor.label)).not.toContain("光污染影响");
    expect(astro?.publicDisplay.factorChips.map((chip) => chip.key)).toEqual([
      "light-pollution",
      "terrain-horizon",
    ]);
    ...
    expect(firstCoveredNight?.judgmentSummary).toMatchObject({
      label: "推荐依据",
      semanticKey: "recommendation_basis",
    });
    expect(firstCoveredNight?.blockerReasons).toEqual([]);
    expect(firstCoveredNight?.judgmentSummary.value).toContain("推荐银河窗口可用");
    expect(firstCoveredNight?.judgmentSummary.value).not.toMatch(/01:10|03:30|主要阻碍/);
    expect(firstCoveredNight?.recommendationBasis.join(" ")).not.toMatch(/01:10|03:30/);
    expect(JSON.stringify(astro)).not.toMatch(/QWeather|Open-Meteo|meteoblue|Amap|和风天气|高德/i);
  });

  it("uses neutral astro daily judgment labels for backup nights with usable windows", () => {
    const base = resultWithAstroHourlyRange("48h", 48);
    const backupAssessment = astroAssessmentForTest({
      practicalAstroScore: 34,
      skyConditionScore: 40,
      milkyWayGeometryScore: 44,
      moonlightImpactScore: 60,
      astroShootable: false,
      recommendedMilkyWayWindow: undefined,
      labels: {
        astronomicalWindow: "有",
    ...
    const result: ForecastCalculationResult = {
      ...
      astroAnalysis: {
        ...base.astroAnalysis,
        dailyAstro: base.astroAnalysis.dailyAstro.map((day, index) =>
          index === 0
            ? {
                ...firstDay,
                starsScore: 42,
                milkyWayScore: 44,
                astroConditionScore: 45,
                astroPracticalScore: 34,
                ...dailyAstroFieldsForTest(backupAssessment),
                astroShootable: false,
                weatherBlockers: [],
                recommendedMilkyWayWindow: undefined,
                recommendationLabel: "谨慎参考",
                keyReason: "银心几何窗口存在，但综合可拍性只适合作为备选。",
                riskNote: "仅作备选",
              }
            : day,
        ),
    ...
    const viewModel = buildAstroForecastViewModel(result);
    const night = viewModel.nightlyCards.find((item) => item.localEveningDate === firstDay.date);

    expect(night?.recommendationLevel).toBe("backup");
    expect(night?.judgmentSummary).toMatchObject({
      label: "关键判断",
      semanticKey: "key_judgment",
    });
    expect(night?.judgmentSummary.value).toContain("银心几何窗口存在");
    expect(night?.judgmentSummary.value).not.toContain("主要阻碍");
  });
  ...
  it("groups WA and VIIRS professional light-pollution data with raw codes collapsed", () => {
    ...
    const primaryGroupsText = JSON.stringify(groups.filter((group) => !group.developerDiagnostics));
    ...
    expect(primaryGroupsText).not.toMatch(
      /wa_model_baseline_available|raw conversion note|raw uncertainty note/,
    );
  ...
  it("renders the astro result as a decision-first dashboard with compact day cards and grouped professional data", () => {
    ...
    expect(viewModel.decisionSummary).toMatchObject({
      ...
      bestWindowLabel: expect.any(String),
      arrivalLabel: expect.any(String),
      directionLabel: expect.any(String),
      mainRiskLabel: expect.any(String),
      confidenceLabel: expect.any(String),
      oneSentenceAdvice: expect.any(String),
    });
    expect(viewModel.actionPlan.map((item) => item.key)).toEqual([
      "timing",
      "window",
      "direction",
      "avoid-direction",
  ...
  it("deduplicates public astro terrain and light-pollution ownership", () => {
    ...
    expect(new Set(viewModel.publicDisplay.factorChips.map((chip) => chip.semanticKey)).size).toBe(
      viewModel.publicDisplay.factorChips.length,
    );
    expect(viewModel.professionalDataGroups.map((group) => group.key)).toEqual(
