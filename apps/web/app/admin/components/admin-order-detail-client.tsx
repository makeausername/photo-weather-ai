      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
        <Card className="grid gap-4 p-5">
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="break-all text-2xl font-bold text-foreground">{detail.order.orderNo}</h2>
                {statusBadge(detail.order.status)}
              </div>
              <p className="mt-2 text-sm text-muted-foreground">
                {providerLabel(detail.order.provider)} / {detail.order.productCode}
              </p>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <InfoItem label="支付流水" value={detail.order.providerTradeNo ?? "-"} />
            <InfoItem label="权益发放" value={detail.order.entitlementGrantedAt ? "已发放" : "未发放"} />
        ...
        <Card className="grid gap-4 p-5">
          ...
          {canSimpleOperate(detail) ? (
            ...
            <Badge variant="muted" className="w-fit">当前状态不支持简单操作</Badge>
          )}
          <FormField label="管理员备注">
            <Textarea value={adminNote} onChange={(event) => setAdminNote(event.target.value)} />
          </FormField>
          <Button variant="secondary" onClick={saveNote}>保存备注</Button>
        </Card>
      </div>
        ...
        <Card className="grid gap-4 p-5">
          ...
          {detail.user ? (
            <div className="grid gap-2 text-sm">
              <Link className="font-semibold text-primary" href={`/admin/users/${encodeURIComponent(detail.user.id)}`}>
                {detail.user.displayName ?? detail.user.email ?? detail.user.phone ?? detail.user.id}
              </Link>
              <p className="text-muted-foreground">{detail.user.email ?? "-"} / {detail.user.phone ?? "-"}</p>
        ...
        <Card className="grid gap-4 p-5">
          ...
          {detail.product ? (
            <div className="grid gap-2 text-sm">
              ...
              <p className="text-muted-foreground">{detail.product.code}</p>
      ...
      <Card className="grid gap-4 p-5">
        ...
        <div className="grid gap-3">
          {detail.timeline.map((item, index) => (
            <div key={`${item.type}-${item.at}-${index}`} className="grid gap-1 rounded-md border border-border p-3">
        ...
        <Table aria-label="支付通知">
          ...
          <tbody className="divide-y divide-border">
            {detail.notifications.map((item) => (
              <tr key={item.id}>
                <td className="px-3 py-2.5">{providerLabel(item.provider)}</td>
                <td className="px-3 py-2.5">{item.status}</td>
                <td className="px-3 py-2.5">{item.signatureVerified ? "通过" : "未通过"}</td>
                <td className="px-3 py-2.5">{item.providerTradeNo ?? "-"}</td>
