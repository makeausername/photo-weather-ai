...
当前公开首页的地图、云层、地形和时间线仍是界面占位，只用于展示产品方向，不包含真实地图图层、真实天气图层或真实服务商调用。当前搜索与 forecast 流程已完成地点识别、坐标归一化、机位匹配、预报范围选择、首页综合判断入口、专题页固定分析目标、本地 mock 天气/地形预报输入构造、本地天文计算、摄影评分和 `/forecast` 结果展示；后续真实天气/地形 provider 接入、查询历史、收藏机位、支付和生产判断逻辑尚未实现。
